...
const getKintoneAuth = (domain, username, password) => {
  ...
  const kintoneApp = new kintone.App({
    connection
  });
  return kintoneApp;
};
...
const addAllFormFields = (app, fields, kintoneApp) => {
  return kintoneApp.addFormFields({
    app,
    fields
  }).then(rsp => {
    return rsp;
...
const updateSpecialFields = (app, fields, kintoneApp) => {
  return kintoneApp.updateFormFields({
    app,
    fields
  }).then(rsp => {
    return rsp;
...
const deployApp = (app, revision, kintoneApp) => {
  ...
  const revert = false;
  return kintoneApp.deployAppSettings({
    apps,
    revert
  }).then(rsp => {
    return rsp;
...
const createNewApp = (name, kintoneApp) => {
  return kintoneApp.addPreviewApp({
    name
  }).then(rsp => {
    return rsp;
  }).catch(err => {
...
const getDeployStatus = (apps, kintoneApp) => {
  return kintoneApp.getAppDeployStatus({
    apps
  }).then(rsp => {
    return rsp.apps;
  }).catch(err => {
...
const updateFormLayout = (app, layout, kintoneApp) => {
  return kintoneApp.updateFormLayout({
    app,
    layout
  }).then(rsp => {
    return rsp;
...
const getFormLayoutApp = (app, kintoneApp) => {
  return kintoneApp.getFormLayout({
    app
  }).then(rsp => {
    return rsp.layout;
  }).catch(err => {
