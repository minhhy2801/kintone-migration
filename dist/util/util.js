  ...
  for (const key in fieldsCopy) {
    if (fieldsCopy.hasOwnProperty(key)) {
      ...
      if (element.type == config.SPECIAL_FIELDS.REFERENCE_TABLE) {
        if (element.referenceTable.relatedApp.app != appCopy) listAppIds.push(element.referenceTable.relatedApp.app);
      } else if (element.hasOwnProperty('lookup')) {
        if (element.lookup.relatedApp.app != appCopy) listAppIds.push(element.lookup.relatedApp.app);
        ...
        for (const k in fields) {
          ...
          if (ele.hasOwnProperty('lookup')) {
            if (ele.lookup.relatedApp.app != appCopy) listAppIds.push(ele.lookup.relatedApp.app);
          }
        }
      }
    }
