...
const createListApps = async listAppIds => {
  try {
    ...
    for (let i = 0; i < listAppIds.length; i++) {
      listCreatedApp[listAppIds[i]] = await migrateNewApp.run(listAppIds[i], config.TARGET.DOMAIN, config.TARGET.USERNAME, config.TARGET.PASSWORD, config.MIGRATE.DOMAIN, config.MIGRATE.USERNAME, config.MIGRATE.PASSWORD);
    }
